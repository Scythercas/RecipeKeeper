...
- フィルタ: カテゴリー(ジャンル)の複数選択(すべて含むレシピを表示)+ 使う食材での絞り込み(複数指定可、カンマ区切り)+ 除外したい食材の指定(いずれか含むレシピを非表示)+ 全文検索。カテゴリーは新規作成可能
- 作った回数の記録: 詳細画面の「作った！」ボタンでワンタップ記録。詳細画面を開いている間は画面がスリープしない(調理中に手が離れてロックするのを防止)
...
- **食材フィルタは文字列の部分一致**です。「鶏」で鶏もも・鶏むねに一致する半面、表記ゆれ(「たまねぎ/玉ねぎ」)は別物扱いになります。厳密にやるなら食材マスタの正規化が必要です。除外フィルタ(「これは使いたくない」食材の指定)も同じ部分一致の仕組みです。
