...
`src/web/screens/SettingsScreen.tsx` は2026年7月に全体をカード型レイアウトへリデザインした。背景`#F5F1EA`の上に、白背景・角丸14・薄いシャドウのカードを並べる方式。セクション順は上から「アカウント」「AIポイント(金色タイントのカード、絵文字+大きな数字で強調)」「パスワード(`/change-password`へ遷移するボタンのみ、入力欄自体は無い)」「常備調味料(タップで開閉するcollapsible、デフォルトは閉じた状態)」「アカウントの削除(意図的に最下部)」。パスワード変更は`src/web/screens/ChangePasswordScreen.tsx`(+ `app/change-password.tsx`のフォールバックと`app/change-password.web.tsx`のReact.lazyシム、`WebRootLayout.tsx`の`Stack`に登録)という別画面に分離し、成功時は`useToast`でトースト表示後`router.back()`で設定画面に戻る(以前は設定画面にインラインで3つのパスワード入力欄が常に表示されていた)。

このカード型の見た目は好評だったため、2026年7月にアプリ全体へ展開した。`src/theme.ts`に`colors`(背景`#F5F1EA`、カード白+ボーダー`#EDE7DD`、アクセント`#007AFF`、破壊的操作`#ff3b30`、金色`#b8860b`系など)と`cardStyle`(角丸14・薄いシャドウ・ボーダーのベーススタイル)をまとめてあり、`StyleSheet.create`内で`{ ...cardStyle, padding: 16, ... }`のように展開して使う。適用範囲: レシピ一覧(`app/(tabs)/index.tsx`、検索・フィルタ一式を1枚のカードにまとめ、各レシピ行もカード化)、`RecipeRow`/`SwipeableRow`(スワイプで見える削除ボタンの右端だけ角丸を合わせている)、レシピ詳細(`app/recipe/[id]/index.tsx`、各セクションがカード)、`RecipeForm`(新規/編集共通)、AIで作るタブ(`app/(tabs)/ai.tsx`)、Web版の認証画面一式(Login/Signup/ForgotPassword/ResetPassword/ChangePassword、いずれもクリーム背景の上にフォームをカードで浮かせる構成に統一)。**ネイティブ版にも同じ変更が及ぶ**(これらは`app/`直下の共有ファイルで`.web.tsx`分岐が無いため)。`PhotoCarousel`は元々`Dimensions.get('window').width`で画面幅いっぱいに表示していたが、カード内にネストされることになったため`onLayout`で実際の描画幅を測る方式に変更した(合わせて角丸をカードと揃えた)。
